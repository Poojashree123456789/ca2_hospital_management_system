...
def init_database():
    ...
    users = [
        User(username='admin', password=generate_password_hash(
            'admin123'), role='admin'),
        User(username='reception', password=generate_password_hash(
            'reception123'), role='receptionist')
    ]
    for user in users:
        db.session.add(user)
    ...
    patients = [
        ...
        Patient(name='Bob Martinez', age=45, gender='Male',
...
def appointments():
    if current_user.role == 'doctor' and current_user.doctor_id:
        appointments_list = Appointment.query.filter_by(
            doctor_id=current_user.doctor_id).order_by(Appointment.date.desc()).all()
    else:
...
def prescriptions():
    if current_user.role == 'doctor' and current_user.doctor_id:
        prescriptions_list = Prescription.query.filter_by(
            doctor_id=current_user.doctor_id).order_by(Prescription.date.desc()).all()
    else:
        prescriptions_list = Prescription.query.order_by(
            Prescription.date.desc()).all()
...
def add_prescription():
    if request.method == 'POST':
        doctor_id = current_user.doctor_id if current_user.role == 'doctor' else int(
